...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int k=0; k<=ke+ngh; k++) {
   for (int j=0; j<=je+ngh; j++) {
    for (int i=0; i<=ie+nhg; i++) { 
         ruser_meshblock_data[0](k,j,i) = b.x3f(k,j,i);
         ruser_meshblock_data[1](k,j,i) = ;
         ruser_meshblock_data[2](k,j,i) = ;
   } } }
  ...
  for (int k=ks; k<=ke; k++) {
   for (int j=js; j<=je; j++) {
    for (int i=is; i<=ie; i++) {
      ...
      pfield->b.x1f(k,j,i) = ((x0/std::pow((std::pow(x0,2.0)+std::pow(y0,2.0)+std::pow(z0,2.0)),1.5)*B_polar)-(xc0/std::pow((std::pow(xc0,2.0)+std::pow(yc0,2.0)+std::pow(zc0,2.0)),1.5)*B_polar*1.52))/b0;
      pfield->b.x2f(k,j,i) = ((y0/std::pow((std::pow(x0,2.0)+std::pow(y0,2.0)+std::pow(z0,2.0)),1.5)*B_polar)-(yc0/std::pow((std::pow(xc0,2.0)+std::pow(yc0,2.0)+std::pow(zc0,2.0)),1.5)*B_polar*1.52))/b0;
      pfield->b.x3f(k,j,i) = ((0.001+z0/std::pow((std::pow(x0,2.0)+std::pow(y0,2.0)+std::pow(z0,2.0)),1.5)*B_polar)-(zc0/std::pow((std::pow(xc0,2.0)+std::pow(yc0,2.0)+std::pow(zc0,2.0)),1.5)*B_polar*1.52))/b0;
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
  ...
  if (mesh_bcs[BoundaryFace::inner_x2] == GetBoundaryFlag("close")) {
